  ...
  (error) => {
    ...
    if (error.response) {
      ...
      if (status === 401) {
        ...
        console.error(`Client error: ${status} - ${error.response.data.message || "Unknown error"}`);
        alert(error.response.data.message || "A client-side error occurred.");
      } else if (status >= 500) {
        // Server Errors 
        console.error(`Server error: ${status} - ${error.response.data.message || "Unknown error"}`);
